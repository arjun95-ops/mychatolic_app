...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _performReverseLookup() async {
     try {
       ...
       // Non-fatal error, filtering might just need manual re-selection
     }
  }
  ...
  Future<void> _pickImage() async {
    ...
    if (picked != null) {
      ...
      if (croppedFile != null) setState(() => _avatarFile = File(croppedFile.path));
    }
  }
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      if (_avatarFile != null) {
        ...
        await _supabase.storage.from('avatars').upload(path, _avatarFile!, fileOptions: const FileOptions(upsert: true));
      ...
      await _supabase.auth.updateUser(UserAttributes(data: { 'full_name': _nameController.text.trim() }));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: bgMain,
      appBar: AppBar(
        title: Text("Edit Profil", style: GoogleFonts.outfit(color: textPrimary, fontWeight: FontWeight.bold)),
        backgroundColor: bgMain,
        elevation: 0,
        centerTitle: true,
        leading: IconButton(
        ...
        actions: [
          TextButton(
             ...
             child: _isLoading 
                ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(color: primaryBrand, strokeWidth: 2))
                : Text("SIMPAN", style: GoogleFonts.outfit(color: primaryBrand, fontWeight: FontWeight.bold)),
          )
        ],
          ...
          child: Column(
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  Text("Data Pribadi", style: GoogleFonts.outfit(color: primaryBrand, fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                  _buildTextField("Nama Lengkap", _nameController, capitalize: true),
                  const SizedBox(height: 16),
                  _buildTextField("Bio / Deskripsi", _bioController, maxLines: 3),
                  const SizedBox(height: 16),
                  _buildLabel("Jenis Kelamin"),
                  _buildDropdown(["Laki-laki", "Perempuan"], _selectedGender, (val) => setState(() => _selectedGender = val!)),
                  const SizedBox(height: 16),
                  _buildLabel("Tanggal Lahir"),
                  _buildDatePicker(),
                  const SizedBox(height: 16),
                  _buildTextField("Suku / Asal", _ethnicityController, capitalize: true),
                  ...
                  // DOMISILI GEREJAWI
                  Text("Domisili Gerejawi", style: GoogleFonts.outfit(color: primaryBrand, fontSize: 18, fontWeight: FontWeight.bold)),
                  Padding(
                    padding: const EdgeInsets.only(top: 4, bottom: 12),
                    child: Text("Data Gereja dapat diubah jika Anda pindah domisili.", style: GoogleFonts.outfit(color: textSecondary, fontSize: 13)),
                  ),
                  ...
                  _buildSearchableField(
                    ...
                    () {
                      _showSearchableSelection(
                        ...
                        filterColumn: null,
                        filterValue: null,
                        dummyData: ["Indonesia", "Timor Leste", "Singapura", "Malaysia", "USA"],
                        onSelect: (item) {
                           setState(() {
                  ...
                  _buildSearchableField(
                    ...
                    () {
                      ...
                      _showSearchableSelection(
                        ...
                        dummyData: ["Keuskupan Agung Jakarta", "Keuskupan Bandung", "Keuskupan Surabaya"],
                        onSelect: (item) {
                           setState(() {
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: _isLoading 
                        ...
                        : Text("SIMPAN PERUBAHAN", style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 16, letterSpacing: 1, color: Colors.white)),
                    ),
                  ),
                  const SizedBox(height: 40),
                ],
      ...
      child: Stack(
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(shape: BoxShape.circle, color: bgSurface, border: Border.all(color: borderLight, width: 2)),
            child: ClipOval(
                child: _avatarFile != null 
                    ? Image.file(_avatarFile!, fit: BoxFit.cover)
                    : SafeNetworkImage(
                        imageUrl: (_avatarUrl != null && _avatarUrl!.isNotEmpty) ? _avatarUrl : "https://i.pravatar.cc/300",
                        fit: BoxFit.cover, fallbackColor: bgSurface, fallbackIcon: Icons.person
                    )
            ),
          ),
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: const BoxDecoration(color: primaryBrand, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black26, blurRadius: 4)]),
  ...
  Widget _buildTextField(String label, TextEditingController controller, {int maxLines = 1, bool capitalize = false}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildLabel(label),
          ...
          child: AbsorbPointer( 
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: primaryBrand, width: 1.5)),
                contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
                suffixIcon: const Icon(Icons.arrow_drop_down, color: textSecondary),
              ),
            ),
          ),
        ),
      ...
      return GestureDetector(
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              Text(
                _selectedBirthDate == null ? "Pilih Tanggal" : DateFormat("dd MMM yyyy").format(_selectedBirthDate!),
  ...
  Widget _buildLabel(String text) {
    return Padding(
      ...
      child: Text(text, style: GoogleFonts.outfit(color: textSecondary, fontSize: 13, fontWeight: FontWeight.w600)),
    );
  }
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(child: Text(widget.title, style: GoogleFonts.outfit(color: Colors.black, fontSize: 18, fontWeight: FontWeight.bold), textAlign: TextAlign.center)),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Colors.transparent)),
              enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: Colors.transparent)),
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: primaryBrand)),
            ),
          ),
          const SizedBox(height: 16),
          Expanded(
            child: _loading 
              ...
              : _results.isEmpty 
                  ? Center(child: Text("Data tidak ditemukan", style: GoogleFonts.outfit(color: Colors.grey)))
                  : ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          title: Text(item['name'], style: GoogleFonts.outfit(color: Colors.black)),
                          onTap: () {
