...
class _PostDetailPageState extends State<PostDetailPage> {
  ...
  Future<void> _toggleLike() async {
    ...
    try {
      if (wasLiked) {
        ...
        await _supabase.from('post_likes').delete().eq('user_id', user.id).eq('post_id', postId);
        await _supabase.rpc('decrement_likes', params: {'row_id': postId});
      } else {
        // WAS NOT LIKED -> LIKE ACTION
        await _supabase.from('post_likes').insert({'user_id': user.id, 'post_id': postId});
  ...
  Future<void> _addComment() async {
    ...
    try {
      ...
      // 2. Increment Counter
      await _supabase.rpc('increment_comments', params: {'row_id': widget.postData['id']});
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Gagal komentar: $e")));
    } finally {
  ...
  Future<void> _confirmDelete() async {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text("Hapus Postingan?", style: TextStyle(fontWeight: FontWeight.w900)),
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                await _supabase.from('posts').delete().eq('id', widget.postData['id']);
                if (mounted) Navigator.pop(context); // Close Page
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Postingan", style: TextStyle(fontWeight: FontWeight.w900)),
        centerTitle: true,
                  ...
                  children: [
                    ...
                    if (hasImage) 
                      ...
                      Container(
                        ...
                        child: Text(
                          content,
                          style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 24, height: 1.2),
                        ...
                        children: [
                          ...
                          GestureDetector(
                            ...
                            child: AnimatedSwitcher(
                              ...
                              child: Icon(
                                _isLiked ? Icons.whatshot : Icons.local_fire_department_outlined,
                                key: ValueKey(_isLiked),
                                color: _isLiked ? Colors.deepOrange : Colors.black,
                                size: 30,
                              ),
                            ),
                          ),
                          ...
                          const Spacer(),
                          const Icon(Icons.bookmark_border, color: Colors.black, size: 30),
                        ],
                      ),
                    ),
                      ...
                      Padding(
                        ...
                        child: RichText(
                          text: TextSpan(
                            ...
                            children: [
                              TextSpan(text: "$creator ", style: const TextStyle(fontWeight: FontWeight.w900)),
                    ...
                    Padding(
                      ...
                      child: Text(
                        timeago.format(DateTime.parse(widget.postData['created_at']), locale: 'id'),
                    ...
                    const Padding(
                      ...
                      child: Text("Komentar", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 14)),
                    ),

                    // 5. Comments List
                    if (_isLoadingComments)
                      const Padding(padding: EdgeInsets.all(20), child: Center(child: CircularProgressIndicator(color: Colors.black)))
                    else if (_comments.isEmpty)
                      const Padding(
                         padding: EdgeInsets.all(20), 
                         child: Center(child: Text("Belum ada komentar. Jadilah yang pertama!", style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic)))
                      )
                    else  
                      ListView.separated(
                        shrinkWrap: true,
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Row(
                            ...
                            children: [
                              const CircleAvatar(
                                backgroundColor: Colors.black, radius: 12,
                              ...
                              Expanded(
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text(c['user_name'] ?? 'User', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                                    const SizedBox(height: 2),
                                    Text(c['content'] ?? '', style: const TextStyle(fontSize: 13)),
                                    const SizedBox(height: 4),
                                    Text(
                                       timeago.format(DateTime.parse(c['created_at']), locale: 'id'),
                ...
                children: [
                  ...
                  _isPostingComment 
                     ...
                     : IconButton(
                        ...
                        icon: const Icon(Icons.send_rounded, color: Colors.black, size: 32),
                      ),
                ],
              ),
            ),
