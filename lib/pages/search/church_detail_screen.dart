...
class _ChurchDetailScreenState extends State<ChurchDetailScreen> {
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      label: Text("Lihat di Peta", style: GoogleFonts.outfit(color: _brandBlue)),
                  ...
                  Text("Jadwal Misa", style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold)),
                ],
              ),
            ),
          ),
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
           return Center(child: Padding(padding: EdgeInsets.all(20), child: Text("Gagal memuat jadwal.")));
        }
        ...
        if (schedules.isEmpty) {
           return Center(
             child: Padding(
               padding: const EdgeInsets.all(20),
               child: Text("Belum ada jadwal misa.", style: GoogleFonts.outfit(color: Colors.grey)),
             ),
           );
        }
         ...
         children: [
           ...
           Wrap(
             ...
             children: items.map((item) {
               final timeString = item['time_start']?.toString().substring(0, 5) ?? "00:00"; // Take HH:mm
               ...
               final isJoined = (isoKey != null && _joinedRadars.containsKey(isoKey));
               final chatGroupId = isJoined ? _joinedRadars[isoKey] : null;

               final boxColor = isJoined ? _brandBlue : Colors.white;
               final borderColor = isJoined ? _brandBlue : _brandBlue.withOpacity(0.3);
               ...
               return InkWell(
                 ...
                 child: Container(
                   ...
                   decoration: BoxDecoration(
                     ...
                     borderRadius: BorderRadius.circular(12),
                     boxShadow: [
                       BoxShadow(color: _brandBlue.withOpacity(0.05), blurRadius: 4, offset: const Offset(0, 2))
                     ]
                   ),
                   child: Row(
                       ...
                       Column(
                         children: [
                           ...
                           Text(
                             ...
                             style: GoogleFonts.outfit(fontSize: 10, color: subTextColor),
                           ),
                         ],
                       ),
  ...
  void _openChat(String chatId, String dayName, String timeString) {
      ...
      final groupName = "Misa $dayName $timeString - $churchName"; // Reconstruct name locally or fetch
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) => SocialChatDetailPage(
            ...
            opponentProfile: {
              'full_name': groupName,
              'avatar_url': "https://id.pinterest.com/herlambanggunaw/gambar-gereja/",
              'is_group': true,
            },
          ),
        ),
  ...
  Future<void> _handleJoinRadar(String dayName, String timeString) async {
    ...
    if (nextMassDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Format hari/jam tidak valid.")));
      return;
    }
    ...
    try {
      ...
      if (existingChat != null) {
        ...
        // 3. CREATE NEW GROUP
        final avatarUrl = "https://id.pinterest.com/herlambanggunaw/gambar-gereja/"; 
      ...
      final existingRadar = await _supabase.from('radars').select().eq('user_id', user.id).eq('church_id', widget.churchData['id']).eq('schedule_time', eventDateIso).maybeSingle();

      if (existingRadar == null) {
        final expiresAt = nextMassDate.add(const Duration(hours: 24)).toIso8601String();
        ...
        if (mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Berhasil bergabung ke Radar & Grup Misa!"), backgroundColor: Colors.green)
          );
        }
      } else {
         if (mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Membuka Grup Misa..."), backgroundColor: _brandBlue)
          );
         }
      }
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Gagal bergabung: $e")));
    }
  }
  ...
  DateTime? _getNextMassDate(String dayNameIndo, String timeString) {
    try {
      ...
      switch (dayNameIndo.toLowerCase()) {
        case 'senin': targetWeekday = DateTime.monday; break;
        case 'selasa': targetWeekday = DateTime.tuesday; break;
        case 'rabu': targetWeekday = DateTime.wednesday; break;
        case 'kamis': targetWeekday = DateTime.thursday; break;
        case 'jumat': targetWeekday = DateTime.friday; break;
        case 'sabtu': targetWeekday = DateTime.saturday; break;
        case 'minggu': targetWeekday = DateTime.sunday; break;
        default: return null; 
      }
      ...
      if (daysToAdd == 0) {
        ...
        final massTimeToday = DateTime(now.year, now.month, now.day, int.parse(timeParts[0]), int.parse(timeParts[1]));
  ...
  Future<void> _launchMap(String address) async {
    ...
    final googleMapUrl = "https://www.google.com/maps/search/?api=1&query=$query";
