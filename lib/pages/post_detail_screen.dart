...
class _PostDetailScreenState extends State<PostDetailScreen> {
  ...
  void _setupRealtimeSubscription() {
    ...
    _subscription!.onPostgresChanges(
      ...
      callback: (payload) {
         print("🔔 Realtime COMMENT Update: $payload");
    ...
    _subscription!.subscribe((status, error) {
       if (status == RealtimeSubscribeStatus.subscribed) {
         print("✅ REALTIME CONNECTED: $channelName");
       } else if (status == RealtimeSubscribeStatus.closed) {
         print("❌ REALTIME CLOSED");
       } else {
         print("ℹ️ REALTIME STATUS: $status ${error ?? ''}");
       }
    });
  }
  ...
  void _handleOptimisticCommentInsert(PostgresChangePayload payload) {
      try {
        ...
        final dummyProfile = Profile(id: json['user_id'].toString(), fullName: "User Baru", role: "Umat");
