...
class _SocialChatDetailPageState extends State<SocialChatDetailPage> with SingleTickerProviderStateMixin {
    ...
    return GestureDetector(
      ...
      child: Column(
        children: [
          ...
          Text(label, style: GoogleFonts.outfit(fontSize: 14, color: AppColors.textPrimary)),
        ],
      ),
    );
  }
  ...
  Future<void> _handleAttachment(ImageSource source) async {
    try {
      ...
      final fileName = 'chat_${DateTime.now().millisecondsSinceEpoch}_$myId.$fileExt';
      final path = '$fileName';

      await _supabase.storage.from('chat-uploads').upload(path, file);

      // 3. Get Public URL
      final imageUrl = _supabase.storage.from('chat-uploads').getPublicUrl(path);
  ...
  Future<void> _handleLocation() async {
    try {
      ...
      if (status.isGranted) {
       ...
       if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Gagal ambil lokasi: $e")));
    }
  }
  ...
  Future<void> _sendMessage({String type = 'text', String? content}) async {
    ...
    try {
      ...
      else if (type == 'story_like') summaryMsg = '🔥 Menyukai story';
      else if (type == 'image') summaryMsg = '📷 Mengirim gambar';
      else if (type == 'location') summaryMsg = '📍 Mengirim lokasi';
      else if (type == 'story_reply') summaryMsg = '💬 Membalas story';

      await _supabase.from('social_chats').update({
        'updated_at': DateTime.now().toIso8601String(),
        'last_message': summaryMsg,
        'participants': [myId, widget.opponentProfile['id']], // Ensure participants are sync
      }).eq('id', widget.chatId);
  ...
  Widget _buildMessageList() {
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        if (!snapshot.hasData) return const Center(child: CircularProgressIndicator(color: AppColors.primaryBrand));
        ...
        if (messages.isEmpty) return Center(child: Text("Mulai percakapan...", style: GoogleFonts.outfit(color: AppColors.textSecondary)));
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return _buildTextBubble(isMe, content, time);
          },
        );
      },
    );
  ...
  // 1. Text Bubble
  Widget _buildTextBubble(bool isMe, String content, String? time) {
    return Align(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text(
               content,
               style: GoogleFonts.outfit(color: isMe ? Colors.white : Colors.black, fontSize: 15),
            ),
            const SizedBox(height: 4),
            Text(
               time != null ? timeago.format(DateTime.parse(time), locale: 'id') : "",
               style: GoogleFonts.outfit(color: isMe ? Colors.white70 : Colors.black38, fontSize: 10),
            ),
          ],
        ),
      ),
     ...
     return Align(
       ...
       child: Container(
         ...
         decoration: BoxDecoration(
           ...
           border: Border.all(color: _beebPrimary.withOpacity(0.5), width: 1), 
           borderRadius: BorderRadius.circular(20), 
         ),
         child: Row(
           mainAxisSize: MainAxisSize.min,
           children: [
             const Icon(Icons.bolt_rounded, color: _beebPrimary, size: 20),
             const SizedBox(width: 6),
             Text(
               "BEEB!", 
               style: GoogleFonts.outfit(color: _beebPrimary, fontWeight: FontWeight.bold, fontSize: 14, letterSpacing: 1.0)
             ),
             const SizedBox(width: 8),
             Text(
                time != null ? timeago.format(DateTime.parse(time), locale: 'id') : "",
                style: GoogleFonts.outfit(color: _beebPrimary.withOpacity(0.6), fontSize: 10),
             ),
           ],
         ),
       ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            // Header Context
            Padding(
               padding: const EdgeInsets.only(left: 12, top: 8, right: 12, bottom: 4),
               child: Row(
                 mainAxisSize: MainAxisSize.min,
                 children: [
                   Container(
             ...
             Padding(
               ...
               child: Align(
                 alignment: Alignment.bottomRight,
                   child: Text(
                    time != null ? timeago.format(DateTime.parse(time), locale: 'id') : "",
                    style: GoogleFonts.outfit(color: isMe ? Colors.white60 : Colors.black38, fontSize: 10),
                 ),
               ),
             )
          ],
        ...
        children: [
              ...
              builder: (context, child) {
                return Transform.scale(
                  ...
                  child: Container(
                    ...
                    child: const Icon(Icons.bolt_rounded, color: Color(0xFFFF5722), size: 24),
                  ),
                );
              },
            ),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [AppColors.primaryBrand, AppColors.primaryHover]),
                shape: BoxShape.circle,
                boxShadow: [BoxShadow(color: AppColors.primaryBrand.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 4))]
              ),
